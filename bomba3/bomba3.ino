...
// Configuración IP fija (sin cambios)
...
//==============================
// Declaración del servidor ASÍNCRONO
//==============================
...
// Pines (sin cambios)
//==============================
const int SELECTOR_BOMBA1 =   17;
const int SELECTOR_BOMBA2 =   18;
const int FLOTADOR_CISTERNA = 19;
const int FLOTADOR_TINACO =   21;
const int SENSOR_FLUJO =      22;
const int SENSOR_PRESION =    23;
const int RELE_BOMBA1 = 1;
const int RELE_BOMBA2 = 2;
const int RELE_BOMBA3 = 3;
const int RELE_BOMBA4 = 4;
const int RELE_BOMBA5 = 5;
const int RELE_BOMBA6 = 12;
...
void startAPBackup();
void stopAPBackup();
void configureWebServer(); // Nueva función para organizar las rutas
void encenderBomba1();
void apagarBomba1();
void encenderBomba2();
void apagarBomba2();
...
void setup() {
  ...
  // Iniciar tarea en Núcleo 0 (sin cambios)
...
//==============================
void loop() {
  // ¡Ya no se necesita server.handleClient() en el loop!
  // El servidor asíncrono funciona en segundo plano.
  ensureWiFi();
  delay(2);
}
...
void configureWebServer() {
  // Servir la página principal desde PROGMEM
  server.on("/", HTTP_GET, [](AsyncWebServerRequest *request){
    request->send_P(200, "text/html", ROOT_HTML);
  });

  // API para obtener el estado del sistema en formato JSON
  ...
  // Rutas de control para las bombas
  server.on("/bomba1on", HTTP_GET, [](AsyncWebServerRequest *request){
    encenderBomba1();
    request->redirect("/");
  });

  server.on("/bomba1off", HTTP_GET, [](AsyncWebServerRequest *request){
    apagarBomba1();
  ...
  server.on("/bomba2on", HTTP_GET, [](AsyncWebServerRequest *request){
    encenderBomba2();
    request->redirect("/");
  });

  server.on("/bomba2off", HTTP_GET, [](AsyncWebServerRequest *request){
    apagarBomba2();
    portENTER_CRITICAL(&estadoMux);
  ...
  server.on("/bomba1manualon", HTTP_GET, [](AsyncWebServerRequest *request){
    encenderBomba1Manual();
    request->redirect("/");
  });

  server.on("/bomba1manualoff", HTTP_GET, [](AsyncWebServerRequest *request){
    apagarBomba1Manual();
    request->redirect("/");
  });

  server.on("/bomba2manualon", HTTP_GET, [](AsyncWebServerRequest *request){
    encenderBomba2Manual();
    request->redirect("/");
  });

  server.on("/bomba2manualoff", HTTP_GET, [](AsyncWebServerRequest *request){
    apagarBomba2Manual();
    request->redirect("/");
  });
}
...
//==================================================================
// A PARTIR DE AQUÍ, EL CÓDIGO DE LÓGICA DE HARDWARE
// (TAREA, CONTROL DE BOMBAS, WIFI ROBUSTO)
// NO NECESITA NINGÚN CAMBIO Y PERMANECE EXACTAMENTE IGUAL.
//==================================================================

//==============================
// Tarea Core 0: Control de hardware
//==============================
void TaskControlHardware(void * pvParameters) {
  ...
  for(;;) {
    ...
    if (b1_enc && !m_manual1) {
      if (digitalRead(FLOTADOR_CISTERNA) == HIGH) {
        ...
        portEXIT_CRITICAL(&estadoMux);
        Serial.println("Iniciando verificación de flujo por 5 minutos");
      } else if (v_flujo && (millis() - t_flujo > 300000)) {
        if (digitalRead(SENSOR_FLUJO) == LOW) {
          Serial.println("No se detecto flujo durante 5 min, apagando bomba de Cisterna");
          apagarBomba1();
          portENTER_CRITICAL(&estadoMux);
          bloqueo_bomba1 = true;
          verificando_flujo = false;
          ...
          Serial.println("Flujo detectado, bomba de cisterna sigue operando");
          portENTER_CRITICAL(&estadoMux);
          tiempo_inicio_flujo = millis();
          portEXIT_CRITICAL(&estadoMux);
        }
    ...
    m_manual2 = modo_manual_bomba2;
    portEXIT_CRITICAL(&estadoMux);

    if (b2_enc && !m_manual2) {
      if (digitalRead(FLOTADOR_TINACO) == HIGH) {
        Serial.println("Tinaco lleno, apagando bomba del tinaco");
        apagarBomba2();
        portENTER_CRITICAL(&estadoMux);
        bloqueo_bomba2 = true;
        portEXIT_CRITICAL(&estadoMux);
...
void ensureWiFi() {
  ...
  if (WiFi.getMode() == WIFI_MODE_AP && apBackupActivo) {
    if (reconnectTries % 3 == 0) {
      Serial.println("[AP Backup] Reintentando STA...");
